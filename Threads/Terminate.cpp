 ...
 * separate thread of execution using std::thread to run a function.
 *
...
static void factorial( const std::string& name )
{
    // 0! = 1
    long factorial = 1;
...
int main( int argc, char ** argv )
{
    ...
    std::thread thread( factorial, "factorial" );
    std::cerr << "Created: " << thread.get_id() << std::endl;
    thread.detach();		// must join or aborts
